// ===== Research Library · Global Sidebar Renderer（本地收藏夹版） =====
// 工作流：
// 1. 优先用 window.RS_MANIFEST（由 sidebar-data.js 注入）拿话题数据；
//    没有才 fetch manifest.json —— file:// 下 fetch 被浏览器拦截，
//    <script src> 不受限，所以本地双击打开必须走 RS_MANIFEST 通道
// 2. base path 从 <script src> 自身位置推断（fallback 到 URL 启发式）
...
(function() {
  ...
  function render(manifest) {
    ...
    // Brand（显式 index.html：file:// 下目录 URL 会变成文件列表页）
    var brand = el('a', { class: 'rs-brand', href: BASE + 'index.html' }, [
    ...
    manifest.topics.forEach(function(topic) {
      ...
      // 话题名链到该话题最新一份报告（file:// 下没有目录索引页可链）
      var latest = topic.reports.reduce(function(acc, r) {
        return (!acc || (r.date || '') > (acc.date || '')) ? r : acc;
      }, null);
      var topicHref = latest ? BASE + topic.slug + '/' + latest.file : BASE + 'index.html';
      var topicLink = el('a', { class: 'rs-topic-name', href: topicHref }, [
  ...
  function init() {
    ...
    if (window.RS_MANIFEST && typeof window.RS_MANIFEST === 'object') {
      try {
        render(sanitizeManifest(window.RS_MANIFEST));
