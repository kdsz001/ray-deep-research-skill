...
7. **禁止递归分家（成本闸门，一字不能省）**：每个子 Agent 的 prompt 末尾必须原样附上这句——
   > 你必须亲自完成全部调研，**禁止使用 Agent / Task 工具再派子 Agent**。web-access skill 里的"子 Agent 分治策略"对你不适用（那是主窗口专用）。
   教训：2026-07-09 的晶合集成调研没加这道闸门，4 个子 Agent 层层再分治，膨胀成 71 个 Agent，几分钟耗尽订阅额度。每个 Agent 上岗都有巨额固定上下文开销，串行慢几分钟远比额度爆炸便宜。
...
3. 只 dispatch 缺失 / 不完整的维度，并用一句话告知用户：
   > "检测到上次中断的进度：{N} 个维度中 {X} 个已完成，只补跑剩余 {Y} 个。"
