...
1.5 **歧义分层处理**（解析后先自评：意图 / 标的 / 范围三项里，有没有"选错会导致整份报告作废"的歧义？没有 → 直接进第 2 步。有 → 按歧义类型分层解决，**禁止把三层混为一谈**）：
   - **第 1 层 · 标的歧义 → 自己查，不许问**。重名公司、代码不明、"X"有多个实体——花 30-60 秒搜索/读页面确认候选；只剩一个最可能候选就直接用（开跑时声明"按 XX 理解"）。**凡是搜索能回答的问题，一律不许弹窗问用户**——把"问用户"当偷懒是违规。查完仍有 ≥2 个同样可能的候选 → 升入第 2 层一起问。
   - **第 2 层 · 意图歧义 → 只有用户能答，弹一次选择题**。典型："调研 Cursor"（投资它？还是想复刻它？）——这决定 preset 走向，在用户脑子里，搜不到。用 AskUserQuestion 问，**最多 1 轮、最多 3 题、全部选择题**（每题给推荐项），答完立即开跑，不再追问。
   - **第 3 层 · 范围歧义 → 用 preset 默认 + 透明声明，不问**。"对比哪几家 / 覆盖多深"这类有合理默认的，直接采用默认，但必须在开跑那句话里把假设说出来（如"默认对比 A/B/C 三家，要换随时说"）——用户在调研跑完前随时可纠正，成本近零。
   - **记忆飞轮**：用户纠正过判断（改类型 / 换标的 / 调范围）→ 按记忆规范存一条 feedback 记忆（如"Ray 说'看看 X'默认指投资调研"），下次同类请求直接命中，不再产生同样的歧义。
2. **判类型**：
...
5. **透明告知 + 默认接受**：用一句话告诉用户决策：
   ...
   然后**立即开始 dispatch，不等用户回应**、**不要弹 AskUserQuestion**（仅有的两个例外：① 第 1.5 步第 2 层的意图歧义选择题；② Stage 0.5 复用检查命中已有报告时，先等用户定夺）。用户如果想改才会主动说"用 6 个" / "改用 generic preset"——再调整 + 重新 dispatch。

   这个设计的核心：**默认零摩擦 + 永远透明 + 始终允许 override**。

### Stage 0.5 · 复用检查（避免重复烧钱）

**在 Stage 0 判断之后、Stage 1 dispatch 之前执行。** 这是 Stage 0 "立即 dispatch 不等回应" 的两个例外之一（另一个是 Stage 0 第 1.5 步的意图歧义选择题）——命中已有报告时值得停下来问一句，因为重跑整份成本高（时间 + token）。
...
3. **若存在近期报告（≤ 30 天）**：说"找到相关报告"，不要说"命中已覆盖"，然后给三选项，**等用户回应**：
   > "找到一份相关报告：**{date}** 的《{title}》。要 (a) 先看现成报告 / (b) 基于现成报告做补充调研 / (c) 忽略现成报告，重新完整调研？"
4. **无报告 / 报告较旧（> 30 天）/ 用户选 (c)** → 正常进入 Stage 1，dispatch 全部子 Agent
5. **用户选 (b) 补充调研** → 先读旧报告，判断它已覆盖哪些需求、缺哪些角度；只 dispatch 缺口/时效相关子 Agent（如融资、口碑、排名、流量、最新进展），复用旧报告的结构性章节，整合时生成 `-v2` 补充版
6. **用户选 (a) 先看现成** → 直接 `open` 旧报告，结束本次调研

**共享库复用（若配了 `~/.config/ray-deep-research/exchange.json`）**：本地库检查之后（朋友无 `library_path`、跳过了本地检查的直接走这步），按 `references/exchange.md` 查共享库 registry —— 找到相关报告时给"先看现成 / 补充调研 / 完整重做"三选一。共享库故障绝不阻塞调研。
...
- [ ] 共享库已查（配了 exchange 时；找到相关报告已给"先看现成 / 补充调研 / 完整重做"三选一 + 埋点）；**没有做任何形式的上传或上传询问**
